...
class TestResolveInputs:
    ...
    def test_git_repository_context_renders_input_templates(
        self, monkeypatch: pytest.MonkeyPatch
    ):
        monkeypatch.setattr(
            variables,
            "get_current_git_repository",
            lambda: "https://github.com/example/iac.git",
        )
    ...
    def test_inline_vars_preserve_runfile_stage_one_values_for_stage_two(self):
                ...
                (
                    ...
                    InlineReference(
                        ...
                        data={
                            ...
                            "bucket_name": "{{ inputs.runfile_name }}-{{ stack.name }}",
                        },
                    ),
                )
            ],
        ...
        assert result["bucket_name"] == "stacksmith.yaml-payments"

    def test_stage_two_supports_jinja_statements_in_input_values(self):
        result = resolve_inputs(
            input_layers=[
                (
                    "vars",
                    InlineReference(
                        ...
                        data={
                            "service": "payments",
                            "label": "{% if inputs.service == 'payments' %}prod{% else %}dev{% endif %}",
                        },
                    ),
                )
            ],
