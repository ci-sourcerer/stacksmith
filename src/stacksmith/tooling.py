...
def _resolve_single_tool(
    ...
    with _tool_cache_lock(cache_root, tool_name):
        return _resolve_single_tool_unlocked(
            tool_name,
            tool_cfg,
            cache_root,
            auth_config,
...
def _resolve_single_tool_unlocked(
    tool_name: ToolName,
    tool_cfg: ToolBinaryConfig,
    cache_root: Path,
    auth_config: RemoteAuthConfig | None,
...
def _is_usable_tool_binary(
    ...
    try:
        result = runner_subprocess.run(
            [str(binary_path), *_VERSION_COMMANDS[tool_name]],
            capture_output=True,
            text=True,
            env=os.environ.copy(),
        )
