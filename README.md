...
- `jenkins/stacksmith-helpers.groovy` holds the shared Jenkins execution logic used by the opinionated wrapper.
...
stacksmith ci validate [--gitops-root <root>] [--discovery-mode <folders|flat-files|env-files>] [--workflow-runfile <path>] [--workflow-env-file <path>] [--workflow-validation-report-format <json|csv>]
```
